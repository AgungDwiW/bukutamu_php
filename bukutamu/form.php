		...
		<!--     <form method = "POST" action = {%url 'bukutamu:signout'%} class = "text-left">
		 -->
			...
			<div class="form-group row"> <!-- no HP -->
			  ...
			  <div class="col-sm-9">  
				<input type="number" class="form-control inputsm" name="NoHP" min=0 id="NoHP" placeholder="08xxxxxxxxxx" autocomplete="off" required <?php echo $hp?"readonly":""; ?>   value = <?php echo $hp; ?>    >
			  </div>
			</div>
			<div class="form-group row"> <!-- no HP -->
			...
			<div class=" col-sm-6">
						...
						<input type="button" name="cancel" id = "cancel" class="col-sm-11"  value="Kembali" onclick="location.href = '../index.php';">

			</div>
			<input type="text" hidden name="multi_num" id = "counter_f" value = 0>
			<?php  
				if (!$flag_sign){
			?>
				<div class="col-sm-6">
					<input type="submit" name="submit" id = "submit" class="col-sm-11" value="Masuk">
				</div>
...
<script>
	...
	?>  
	 ...
	 ?>
	 const counter_t = document.getElementById("counter_f");
  ...
  navigator.mediaDevices.getUserMedia(constraints)
   ...
   $("#addtamu").click(function() {
		...
		var tName = $("<div class=\"col-sm-6\">  <input type=\"text\" class=\"form-control inputsm\" placeholder=\"UID "+tcounter+"\" id = \"uidt"+tcounter+"\" name = \"uidt"+tcounter+"\" onkeyup=\"getTamu("+tcounter+")\"> </div>");
		var tType = $("<div class=\"col-sm-3\"><select class=\"form-control inputsm\"  placeholder=\"Tipe id\"required id = tidt"+tcounter+" name = tidt"+tcounter+"><option value=\"KTP\"" + ">KTP</option><option value=\"Kartu Pegawai\"" + ">Kartu Pegawai</option><option value=\"SIM\"" +">SIM</option></select>"); 
		var removeButton = $("<label class=\"control-label col-sm-3\" for=\"UID\">UID Tambahan:</label>);")
		var tname = " <label class=\"control-label col-sm-3\" for=\"Nama\">Nama:</label><div class=\"col-sm-6\"><input type=\"text\" class=\"form-control inputsm \" name=\"Namat"+tcounter+"\" id=\"Namat"+tcounter+"\"  placeholder=\"Nama Tamu "+tcounter+"\" style=\"text-transform:uppercase\" required ></div><div class=\"col-sm-3\"><select class=\"form-control inputsm\" name=\"Kelamint"+tcounter+"\" id=\"Kelamint"+tcounter+"\" placeholder=\"L/P\"   ><option value=\"L\" >L</option><option value=\"P\">P</option></select></div>"
		var thid = "<input type=\"text\" hidden name = \"hidt"+tcounter+"\" id = \"hidt"+tcounter+"\" value = \"-\">";
		removeButton.click(function() {
			$(this).parent().remove();
		});
		fieldWrapper.append(removeButton);
		...
		fieldWrapper.append(tname);
		fieldWrapper.append(thid);
		tcounter++;
		counter_f.value = tcounter;
...
function getTamu(id){
	// console.log(id);
	var uid = document.getElementById("uidt"+id);
	var tid = document.getElementById("tidt"+id);
	var nama = document.getElementById("Namat"+id);
	var kelamin = document.getElementById("Kelamint"+id);
	var hid = document.getElementById("hidt"+id);

	// console.log(uid.value);
  	$.ajax({
		url: "ajax/get_tamu_multi_entry.php", 
		type : 'POST',
		data : {uid:uid.value},
		success: function(result){
			...
			if (!res['error']){
				...
				if (res['tid'] == "KTP") 
					tid.selectedIndex = 0;
				else if (res['tid'] == "Kartu Pegawai") 
					tid.selectedIndex = 1;
				else if (res['tid'] == "SIM") 
					tid.selectedIndex = 2;
				else if (res['tid'] == "Paspor") 
					tid.selectedIndex = 3;
				else if (res['tid'] == "Lainnya") 
					tid.selectedIndex = 4;
				if (res['kelamin']=="L")
					kelamin.selectedIndex = 0;
				else
					kelamin.selectedIndex = 1;
			}
			else{
				nama.value = "";
				hid.value = "-";
